...
```
    ...
    Info: Working in directory /Users/erinjang/Desktop/BIMM143/bimm143_git/class14
...
```
    ...
    Warning in text.default(xloc, yloc, label = labs[sub2grp[sub.idx, 1]], cex = cex): length(labels) > max(length(x), length(y));
    'labels' truncated to length 36
...
The “Cell Cycle” pathway has the most significant “Entities p-value”.
The most significant pathways listed does not seem to match my previous
KEGG results. This may because the two methods, Reactome and KEGG,
organize biology differently. KEGG is known to group genes into broader
metabolic and signaling pathways, whereas Reactome is known to separate
processes into more detailed sub-steps. Reactome calculates with the
overlap between the number of submitted genes and its pathway entities.
KEGG calculates enrichment depending on its own gene sets.
